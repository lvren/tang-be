...
class WxPayApi
{
    ...
    {
        $url = env('PAY_URL') . "/pay/unifiedorder";
        //检测必填参数
        ...
        $result = WxPayData\WxPayResults::Init($config, $response);
    ...
    {
        $url = env('PAY_URL') . "/pay/orderquery";
        //检测必填参数
    ...
    {
        $url = env('PAY_URL') . "/pay/closeorder";
        //检测必填参数
        if (!$inputObj->IsOut_trade_noSet()) {
    ...
    {
        $url = env('PAY_URL') . "/secapi/pay/refund";
        //检测必填参数
        if (!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
    ...
    {
        $url = env('PAY_URL') . "/pay/refundquery";
        //检测必填参数
    ...
    {
        $url = env('PAY_URL') . "/pay/downloadbill";
        //检测必填参数
    ...
    {
        $url = env('PAY_URL') . "/secapi/pay/reverse";
        //检测必填参数
        if (!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
    ...
    {
        $url = env('PAY_URL') . "/payitil/report";
        //检测必填参数
    ...
    {
        $url = env('PAY_URL') . "/tools/shorturl";
        //检测必填参数
    ...
    public static function notify($config, $callback, &$msg)
    {
        ...
        try {
            ...
            $result = WxPayData\WxPayNotifyResults::Init($config, $xml);
    ...
    private static function reportCostTime($config, $url, $startTimeStamp, $data)
    {
        ...
        $objInput = new WxPayData\WxPayReport();
